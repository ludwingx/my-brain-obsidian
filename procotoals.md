...
Hardening de Dispositivos (NO - Limpieza):
...
Gestión de LTE vs WiFi:
Regla de Oro: El registro siempre se hace con la LTE de la línea asignada. El algoritmo detecta que el número de teléfono coincide con la IP de la operadora (Carrier NAT), lo cual aumenta la confianza ("Trust Score") masivamente.
Proxies/Compartidos: Una vez verificada la cuenta (día 2 o 3), se puede migrar al MB compartido, pero nunca en el momento del registro.
...
Huella Digital del Navegador/App:
...
Acción: Crear cuenta Google.
Input: Nombre y Apellido de la identidad creada.
Verificación: Usar el número LTE del chip insertado.
Perfil:
Subir la foto (ya tratada).
...
Seguridad: Guardar los códigos de respaldo en un Excel offline (no en la nube todavía).
FASE 3: Creación y Calentamiento de Facebook (El Objetivo)
Tiempo estimado por cuenta: 30 - 45 minutos (Día 1).
Instalación: Descargar Facebook y Messenger desde la Play Store (usando la cuenta Gmail recién creada). Esto vincula el "Google Advertising ID" del dispositivo con Facebook.
Registro:
...
Nombre: Real.
Verificación: Facebook pedirá el número. Poner el LTE. Ingresar el código.
Perfilado Inmediato:
Subir foto de perfil.  / creado con el protocolo de imagen
Subir foto de portada (Cover Photo). Consejo: Que sea genérica, paisajes, un equipo de fútbol, etc. Evita fotos de personas en portada al inicio.
Añadir "Acerca de": Ciudad actual, ciudad natal. NO poner lugar de trabajo ni estudios el primer día (se ve automatizado).
Acciones de "Calentamiento en Frío" (Día 1):
...
Tiempo invertido: ~10.5 horas. Aquí empieza la fatiga del operador.
...
La Regla del GPS: No actives el GPS los primeros 3 días. El algoritmo suele asociar "Cuenta nueva + GPS activado pidiendo permisos" con bots de granja.
Día 4: Activar GPS, abrir Google Maps, trazar una ruta (simulando que buscas dirección), y cerrar. Luego desactivar.
Apps de Terceros: No instales apps raras. Ten las básicas: WhatsApp, Instagram, tiktok (opcional pero recomendado vincular), Spotify.
...
Comportamiento en YouTube (Crucial):
